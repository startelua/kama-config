...
function ksr_onreply_manage_rtpengine()
	...
	-- add meta data from to  metadata record-call=yes label=Aleg_label 
--[[ local dest_number = KSR.pv.get("$rU")
        local to_uri = KSR.pv.get("$tu");
        local call_id = KSR.pv.get("$ci")
        local from_number = KSR.pv.get("$fU") or ""
...
--    local routing_policy = 4;
	...
	KSR.log("info","from"..KSR.kx.get_furi().." dst "..KSR.pv.get("$ru").." src "..KSR.kx.get_srcip().."  dst uri "..dest_uri.."")
...
-- add record forward {uri.user} $(fU{uri.user}) KSR.pv.get("$fU")
        if (KSR.isflagset(FLT_FROM_PROVIDER)) then
	    KSR.log("err", "RTP from pow row 549 \n")
	    local rtp_option="record-call=yes metadata=from:"..KSR.kx.get_fuser().."|to:"..KSR.kx.get_tuser().." label=calleeR media-echo=both"
	    ...
	    KSR.log("err", "RTP from pow row 553 \n")
       if KSR.rtpengine.rtpengine_manage(rtpengine) > 0 then
...
--	    KSR.rtpengine.rtpengine_manage("start forwarding")
    end
  ...
  --  end
	    ...
	    KSR.log("err", "RTP from pow row 574 \n")
end
--[[---------------- EVENT ]]--
